...
> **Branch hygiene:** `Ingest`, `Console`, and `Exec-Assistant` are each
> trimmed to hold only their own product. Please don't merge one of these
> branches into another (or into `main`) via a PR — the CI/health-check
> workflows and READMEs are intentionally *different* content per branch,
> and a text merge across them will corrupt both (duplicate `on:` blocks,
